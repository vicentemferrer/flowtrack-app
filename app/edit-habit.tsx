...
export default function EditHabitScreen() {
	const { habitUuid } = useLocalSearchParams();
	const { habit, loading: loadingHabit, loadHabitDetails } = useHabitDetails();
	const { categories, updating, error, updateHabit, validateForm } = useHabitUpdate();
	...
	// Load habit details on mount
	useEffect(() => {
		if (habitUuid && typeof habitUuid === 'string') {
			loadHabitDetails(habitUuid);
		}
	}, [habitUuid, loadHabitDetails]);
	...
	useEffect(() => {
		if (habit) {
			setFormData({
				...
				category_uuid: habit.category?.uuid || null
			});
		}
	}, [habit]);
	...
	const handleSubmit = async () => {
		...
		// Update habit
		const success = await updateHabit(habitUuid, formData);
		if (success) {
			...
			<KeyboardAvoidingView
				...
				<View style={styles.header}>
					...
					<Pressable
						...
						disabled={updating}>
						<Text style={[styles.saveButtonText, updating && styles.saveButtonTextDisabled]}>
